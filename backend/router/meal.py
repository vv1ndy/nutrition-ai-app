...
def log_meal(
    ...
    if nutri_record:
        ...
        nutri_record.total_protein += (new_meal.meal_protein_g or 0)
        nutri_record.total_carb += (new_meal.meal_carb_g or 0)
        nutri_record.total_fat += (new_meal.meal_fat_g or 0)
    else:
...
def get_meals_by_date(
    ...
    user_id: int=Depends(get_current_user),
    db: Session = Depends(get_db)):
    """API Lấy danh sách bữa ăn theo một ngày cụ thể (Định dạng: YYYY-MM-DD)"""
    try:
        # Ép chuỗi "YYYY-MM-DD" từ Frontend thành đối tượng ngày tháng
        target_date = datetime.strptime(date, "%Y-%m-%d").date()
        
        print(f"Đang tìm kiếm bữa ăn cho User {user_id} vào ngày: {target_date}")
        
        # Lọc dữ liệu bằng đúng cột ngay_an trong Database của bạn
        meals = db.query(Meal).filter(
            Meal.user_id == user_id,
            Meal.ngay_an == target_date
        ).all()
        nutri_record = db.query(Nutri).filter(Nutri.user_id == user_id, Nutri.date_log == target_date).first()
        target = db.query(TargetNutri).filter(TargetNutri.user_id == user_id).order_by(TargetNutri.target_nutri_id.desc()).first()
        tdee = float(target.target_calories) if target and target.target_calories else 2000
       # ÉP KIỂU SQL OBJECT SANG JSON
        meals_encoded = jsonable_encoder(meals)
        print(f"--- Đã tìm thấy {len(meals)} bữa ăn ---")
        return {
        "meals": meals,
        "consumed_calories": float(nutri_record.total_calories or 0) if nutri_record else 0.0,
        "daily_goal": tdee,
        }
    except Exception as e:
        # Đã có except để bắt lỗi, sẽ không bị SyntaxError nữa
